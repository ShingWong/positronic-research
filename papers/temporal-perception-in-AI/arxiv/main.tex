...
\author{Shing Wong}
\affiliations{Independent Researcher \\ \texttt{swong@persona-bot.com}}
\begin{document}
\maketitle
\begin{abstract}
...
\section{Polytemporal Representation}\label{sec:poly}
...
\paragraph{RULER Efficiency.} Across $4$--$32$k contexts, top-$8$ retrieval uses a flat $242$~tok vs.\ a linearly growing full haystack ($2.2$--$18$k~tok), a $1/10$--$1/74$ reduction, preserving recall@1 $1.0$ (Fig.~\ref{fig:ruler}); full sweep in the RULER Efficiency appendix; see also \texttt{figs/ruler\_efficiency.pdf}.
...
\textbf{only} in the clock driving the prune ladder
($R=e^{-\Delta\tau/S}$ vs.\ wall-clock MemoryBank-style $R=e^{-t/S}$):
polytemporal $\tau$ (novelty-integrated) vs.\ wall-clock $t$. On a uniform stream (control) the two clocks
